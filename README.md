...
서버 없이 **HTML 파일 1개**로 동작하는 한국어 상식 퀴즈 게임. 개인 API 키(BYOK)를 등록하면 AI가 문제를 생성하고, 키가 없어도 내장 기본 문제로 플레이할 수 있습니다.
...
- AI 문제 생성: **OpenAI / Claude / Gemini** 프로바이더 선택, 브라우저 직접 호출 (배치 생성 + 백그라운드 선행 로딩 + 중복 방지)
  - OpenAI: `gpt-4o-mini`, `gpt-4.1-mini`, `gpt-4o`, `gpt-5-mini`
  - Claude: `claude-haiku-4-5`, `claude-sonnet-4-6`
  - Gemini: `gemini-2.5-flash`, `gemini-2.0-flash`, `gemini-2.5-pro` (무료 티어 사용 가능)
- 키는 프로바이더별로 저장하며 설정 화면에서 모델 선택 가능
